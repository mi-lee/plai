...
(require plai-typed/s-exp-match
         (typed-in racket [gensym : (-> s-expression)]))

;; Start with "variable.rkt"
;;
;; Remove `letC`, but ensure that `parse` still
;; supports `{let .....}` with the same meaning
;; as before.
...
(define (parse [s : s-expression]) : ExprC
  (cond
    ...
    [(s-exp-match? '{begin ANY ANY} s)
     (let ([E1 (second (s-exp->list s))]
           [E2 (third (s-exp->list s))])
       (parse `{let {[,(gensym) ,E1]} ,E2}))]
    [(s-exp-match? '{ANY ANY} s)
