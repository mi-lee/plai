...
(module+ test
  ...
  (test/exn (interp (parse `x) mt-env)
            "free variable")
  (test (interp (parse `x) 
                (extend-env (bind 'x (lambda () (numV 9))) mt-env))
        (numV 9))
  ...
  ;; !! A "bad" book-language recursive binding has the same
  ;;    result as bad plai-typed recursive binding, and so
  ;;    have to use `try` to test it:
  (test (try (interp (parse '{letrec {[x x]} x})
                     mt-env)
             (lambda ()
               (numV 7)))
        (numV 7)))
...
(module+ test
  ...
  (test (lookup 'x (extend-env
                    (bind 'x (lambda () (numV 9)))
                    (extend-env (bind 'x (lambda () (numV 8))) mt-env)))
        (numV 9))
  (test (lookup 'y (extend-env
                    (bind 'x (lambda () (numV 9)))
                    (extend-env (bind 'y (lambda () (numV 8))) mt-env)))
        (numV 8)))
