  ...
  (type-case ExprC a
    ...
    [pairC (f s)
           (interp f env
                   (pairSecondK s env k))]
    [fstC (p)
          (interp p env (doFstK k))]
    [sndC (p)
          (interp p env (doSndK k))]))
...
(define (continue [k : Cont] [v : Value]) : Value
  (type-case Cont k
    ...
    [pairSecondK (s env next-k)
                 (interp s env (doPairK v next-k))]
    [doPairK (vl next-k)
             (continue next-k (pairV vl v))]
    [doFstK (next-k)
            (continue next-k
                      (type-case Value v
                        [pairV (f s) f]
...
(module+ test
  ...
  (test (interp (parse '{fst {pair 1 2}})
                ...
                (doneK))
        (numV 1))
  (test (interp (parse '{snd {fst {pair {pair 1 2} 3}}})
                mt-env
                (doneK))
        (numV 2))
  ...
  (test (continue (doMultK (numV 7) (doneK)) (numV 5))
        (numV 35))
  (test (continue (appArgK (numC 5) mt-env (doneK)) (closV 'x (idC 'x) mt-env))
        (numV 5))
  (test (continue (doAppK (closV 'x (idC 'x) mt-env) (doneK)) (numV 8))
        (numV 8)))
