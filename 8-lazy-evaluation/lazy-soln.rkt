...
(module+ test
  ...
  (test/exn (interp (parse `x) mt-env)
            "free variable")
  (test (interp (parse `x) 
                (extend-env (bind 'x (delay (numC 9) mt-env)) mt-env))
        (numV 9))
...
(module+ test
  (test (force (delay (numC 8) mt-env))
        (numV 8))
  (test (force (delay (idC 'x) (extend-env (bind 'x (delay (numC 9) mt-env))
                                           mt-env)))
        (numV 9)))
...
(module+ test
  (test/exn (lookup 'x mt-env)
            "free variable")
  (test (lookup 'x (extend-env (bind 'x (delay (numC 8) mt-env)) mt-env))
        (delay (numC 8) mt-env))
  (test (lookup 'x (extend-env
                    (bind 'x (delay (numC 9) mt-env))
                    (extend-env (bind 'x (delay (numC 8) mt-env)) mt-env)))
        (delay (numC 9) mt-env))
  (test (lookup 'y (extend-env
                    (bind 'x (delay (numC 9) mt-env))
                    (extend-env (bind 'y (delay (numC 8) mt-env)) mt-env)))
        (delay (numC 8) mt-env)))
