...
;; * Add {swap expr expr}
;;
;; Could be sugar, like this:
;;  {swap x y} -> {let {[z {unbox x}]}
;;                   {begin
;;                     {set-box! x {unbox y}}
;;                     {set-box! y z}}}
;; but we implemented `swap` as a core form, instead

;; * Change representation of the store so that `new-loc`
...
(define-type Store
  [store [mem : (listof Storage)]
         [max-address : number]])
(define mt-store (store empty 0))
(define (override-store [c : Storage] [sto : Store])
  (type-case Store sto
    [store (mem max-address)
           (store (cons c mem)
                  (max (cell-location c)
                       max-address))]))
...
(define (parse [s : s-expression]) : ExprC
  (cond
    ...
    [(s-exp-match? '{swap ANY ANY} s)
     (swapC (parse (second (s-exp->list s)))
            (parse (third (s-exp->list s))))]
    [(s-exp-match? '{ANY ANY} s)
  ...
  (type-case ExprC a
    ...
    [swapC (l r)
           (with [(v-l sto-l) (interp l env sto)]
                 (with [(v-r sto-r) (interp r env sto-l)]
                       (type-case Value v-l
                         [boxV (l-l) (type-case Value v-r
                                       [boxV (l-r)
                                             (v*s
                                              (numV 0)
                                              (override-store
                                               (cell l-l (fetch l-r sto-r))
                                               (override-store
                                                (cell l-r (fetch l-l sto-r))
                                                sto-r)))]
                                       [else (error 'interp "not a box")])]
                         [else (error 'interp "not a box")])))]))
...
(module+ test
  ...
  (test/exn (interp (parse '{let {[bad {lambda {x} {+ x y}}]}
                    ...
                    mt-store)
            "free variable")
  (test (interp (parse '{swap {box 1} {box 2}})
                mt-env
                mt-store)
        (v*s (numV 0)
             (override-store (cell 1 (numV 2))
                             (override-store (cell 2 (numV 1))
                                             (override-store
                                              (cell 2 (numV 2))                                            
...
(define (fetch [l : Location] [sto : Store])
  (type-case Store sto
    [store (mem max-addr) (fetch* l mem)]))
