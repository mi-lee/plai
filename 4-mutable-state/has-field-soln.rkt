...
(define (parse [s : s-expression]) : ExprC
  (cond
    ...
    [(s-exp-match? '{has ANY SYMBOL} s)
     (hasC (parse (second (s-exp->list s)))
           (s-exp->symbol (third (s-exp->list s))))]
    [(s-exp-match? '{ANY ANY} s)
...
(module+ test
  ...
  (test/exn (parse '{{+ 1 2}})
            "invalid input")
  (test (parse '{has {+ 1 2} a})
        (hasC (plusC (numC 1) (numC 2)) 'a)))
...
(module+ test
  ...
  (test (interp (parse '{let {[r1 {record {a {+ 1 1}}
                ...
                mt-env)
        (numV 7))
  (test (interp (parse '{has {record {a {+ 1 1}}
                                     {b {+ 2 2}}} a})
                mt-env)
        (boolV true))
  (test (interp (parse '{has {record {a {+ 1 1}}
